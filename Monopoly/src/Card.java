...
public class Card extends Square {
    ...
    public void LuckyDrawChanceCard(Player player, GUIV2 gui) {
        System.out.println("Player " + player.getName() + " got a chance card for a lucky draw.");
        Random random = new Random();
        int prizeAmount = random.nextInt(10000) + 1000; // Random amount between $10000 and $100000
        player.setCash(player.getCash() + prizeAmount);
        System.out.println("Congratulations! You won $" + prizeAmount + " in the lucky draw!");
    }
    
    public void PropertySwapChanceCard(Player player, Board board, GUIV2 gui) {
        System.out.println("Player " + player.getName() + " got a chance card to swap one of their properties with another player.");
        System.out.println("Please choose a property to swap:");
        for (int i = 0; i < player.getOwnedProperty().size(); i++) {
            System.out.println((i + 1) + ". " + player.getOwnedProperty().get(i).getName());
        }
        ...
        while (true) {
            try {
                System.out.println("Please input your choice: ");
                choice = gui.getUserInputNumber();
                if (choice <= player.getOwnedProperty().size() && choice > 0) {
                    PropertySquare propertyToSwap = player.getOwnedProperty().remove(choice - 1);
                    System.out.println("Please choose a player to swap properties with:");
                    for (int i = 0; i < board.getPlayers().size(); i++) {
                        if (!board.getPlayers().get(i).equals(player)) {
                            System.out.println((i + 1) + ". " + board.getPlayers().get(i).getName());
                        }
                    ...
                    while (true) {
                        try {
                            System.out.println("Please input your choice: ");
                            playerChoice = gui.getUserInputNumber();
                            if (playerChoice <= board.getPlayers().size() && playerChoice > 0) {
                                Player otherPlayer = board.getPlayers().get(playerChoice - 1);
                                otherPlayer.getOwnedProperty().add(propertyToSwap);
                                System.out.println("Property " + propertyToSwap.getName() + " swapped with " + otherPlayer.getName() + "'s property.");
                                break;
                            } else {
                    ...
                    System.out.println("Please choose a valid property!");
                }
            } catch (Exception e) {
                System.out.println("Please input a valid integer choice!");
            }
